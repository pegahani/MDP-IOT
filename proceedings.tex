...
\begin{algorithm}[]
 \KwData{VMDP-SC$(AS, A(), P, \bar{r}, \gamma)$, a $W$ polytope of user weights on objectives, precision $\epsilon$}
 ...
 \Repeat{$|| \bar{v}_t - \bar{v}_{t+1} || \leq \epsilon$}{
	...
	\For{\textbf{each} $as \in AS$}{
		...
		\For{\textbf{each} $cs \in A(as)$}{
			$\bar{v}_t(as) \longleftarrow \overline{QoS}(as, cs) + \sum_{as'} P(as' | as, cs) \bar{v}_{t-1}(as')$\\
			$($ best $, \mathcal{K} ) \longleftarrow $ getBest(best, $\bar{v}_t, \mathcal{K}$) \\
		   	$\bar{v}_t(as) \longleftarrow $ best \\
		   	\If{best = $\bar{v}_t(as)$}{
				$\pi_{\text{best}(as) \longleftarrow cs}$
			}
	 }
	}
 } 
